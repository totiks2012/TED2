...
- Подсветка синтаксиса расширена до 7 языков: Tcl, Python, C/C++, JavaScript, HTML, SQL, Bash .
...
- Подсветка синтаксиса для Tcl, Bash, C, C++, Python, JavaScript, HTML, SQL
...
Скачайте архив с проектом и распакуйте По пути `$HOME/.local/bin/TED3+`. Запуск из лаунчера обязателен он обеспекчить открытие из контекстного меню файлового менеджера :

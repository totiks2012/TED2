...
proc ::core::create_tab {{filename ""}} {
    ...
    # Если вкладка открыта через аргумент, устанавливаем фокус на её текстовый виджет.
    if {$from_argument} {
        .tabs select $tab
        focus $tab.text
    }
...
proc ::core::open_file {} {
    ...
    # При открытии файла через диалог, считаем это аргументом
    set ::core::from_argument 1
    set new_tab [::core::create_tab $filename]
    # Сброс флага после открытия всех аргументных файлов
...
proc ::core::init {} {
    ...
    # Если редактор запущен с аргументами (например, файл открыт через контекстное меню файлового менеджера),
    # устанавливаем флаг, открываем каждый переданный файл, и каждый раз фокусируя соответствующую вкладку.
    if {[llength $::argv] > 0} {
        set ::core::from_argument 1
        foreach file $::argv {
            ::core::create_tab $file
        }
        # После открытия всех файлов из аргументов флаг сбрасывается,
        # чтобы последующее создание вкладок (например, из сессии) не меняло фокуса.
        set ::core::from_argument 0
    } else {
        ::core::create_tab
    }
